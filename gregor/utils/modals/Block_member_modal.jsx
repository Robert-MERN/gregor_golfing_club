...
const Block_member_modal = ({ open, close }) => {
    const { member_delete_id, handleUpdateUserAPI } = useStateContext()
    const handle_block = () => {
        handleUpdateUserAPI({ accountStatus: false, _id: member_delete_id }, "", "", "admin_true", "member_edit");
        close();
    }

    return (
        <Dialog
            open={open.block_member_modal}
            onClose={close}
        >
            <div className=' md:w-[500px] p-6 md:p-7 relative flex flex-col gap-8 md:gap-10' >
                ...
                <p className='text-[13px] md:text-[16px] text-stone-600 font-medium' >Are you sure you want to block this Member?</p>
                <div className='w-full flex justify-end gap-4' >
                    ...
                    <button onClick={handle_block} className='bg-red-500 hover:bg-red-400 px-4 py-[6px] rounded-md text-white text-[12px] md:text-[15px] transition-all' >Block</button>
                </div>
            </div>
        </Dialog>
    )
...
export default Block_member_modal